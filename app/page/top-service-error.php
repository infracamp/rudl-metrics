...
$sql = "SELECT top(cnt,http_host,request_uri,status,16) as cnt FROM (SELECT count(bytes_sent) as cnt FROM syslog WHERE time > now() - 15m AND (status='500' OR status='404') group by http_host, request_uri,status)";
